                                ...
                                @foreach ($users as $user)
                                    @if ($user->campaign_id == 4)
                                        <option value="{{ $user->id }}"
                                            @if ($user->id == $user_id) selected @endif>{{ $user->name }}</option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <select name="campaign_id" class="form-control select2">
                                ...
                                @foreach ($campaigns as $campaign)
                                    <option value="{{ $campaign->id }}" @if ($user->id == $campaign_id) selected @endif>
                        ...
                        @foreach ($voice_audits as $audit)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <br>({{ $audit->campaign->name ?? '' }}) - ({{ $audit->project->name ?? '' }})
                                </td>
                                ...
                                <td class="action">
                                    <a href="{{ route('voice-audit-appeals.show', $audit) }}"
                                    ...
                                    @if ((in_array(Auth::user()->roles[0]->name, ['Director', 'Team Lead', 'Manager', 'Associate']) &&
                                        ...
                                        @if ($audit->appeal->status == 'pending')
                                            <a href="{{ route('voice-audit-appeals.edit', $audit) }}"
                                               class="btn btn-primary btn-sm"><i class="fas fa-edit"></i></a>
                                            <form action="{{ route('voice-audit-appeals.destroy', $audit) }}"
                                                  method="post">
                                                @csrf
                                                @method('delete')
                                                <button type="submit" class="btn btn-danger btn-sm"
                                                        onclick="return confirm('Are you sure?')"
                                                        style="padding: 0.25rem 10px;"><i class="fas fa-times"></i></button>
                                            </form>
                                        @endif
                                    @endif
                                </td>
